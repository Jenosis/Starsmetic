...
{
	public class WatermelonStalk : ModTile
	{
		...
		{
			int stage = Main.tile[i, j].frameX / 32;
			if (stage == 2)
			{
				Item.NewItem(i * 16, j * 16, 0, 0, mod.ItemType("WatermelonSeed"));
				Item.NewItem(i * 16, j * 16, 0, 0, mod.ItemType("Watermelon"));
			}
			return false;
		}
		...
		public override void RandomUpdate(int i, int j)
		{
			...
			{
				Main.tile[i, j].frameX += 32;
			}
			else if (Main.tile[i, j].frameX == 32)
			{
				Main.tile[i, j].frameX += 32;
			}
		}
